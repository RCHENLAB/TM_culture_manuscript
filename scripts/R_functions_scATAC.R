...
runcreateSignacObj <- function(dirIn,dirOut,prefix,genomeV){
  ...
  if(genomeV=="hg38"){
    annotations <- GetGRangesFromEnsDb(ensdb = EnsDb.Hsapiens.v86)} else if(genomeV=="mm10"){
      annotations <- GetGRangesFromEnsDb(ensdb = EnsDb.Mmusculus.v79::EnsDb.Mmusculus.v79)} else {
        stop("check genome!")}
  ...
  print("check_2: calculated pct_reads in peaks")
  print(objAt)
  objAt$blacklist_ratio <- objAt$blacklist_region_fragments / objAt$peak_region_fragments
  print("check_3: calculated blacklist ratio")
  print(objAt)
  objAt$high.tss <- ifelse(objAt$TSS.enrichment > tssE, 'High', 'Low')
  print("check_4: calculated tss")
  print(objAt)
  p1<-TSSPlot(objAt, group.by = 'high.tss') + NoLegend() #+ ggtitle(paste0("TSS enrichment: ",prefix))+labs(subtitle=prefix)
  print("check_5: done TSS plot")
  print(objAt)
  objAt$nucleosome_group <- ifelse(objAt$nucleosome_signal > nsr, paste0('NS > ',nsr), paste0('NS < ',nsr))
  
  # if nucleosome_group has a category less than a few either way, the following plot errors out (facet_grid error) because it's only plotting in the region of the first few Mb of chr1
  # we can help this by adjusting the region that we're checking for the group on
  print(table(objAt$nucleosome_group))
  if (any(table(objAt$nucleosome_group) < 5)) {
	print("There are under 5 fragments in one of the nucleosome groups, skipping FragmentHistogram.")
	p2 <- NULL
  } else {
  ...
  png(paste0("01_1_plot_tss_",prefix,".png"),height = 800, width = 1000)
  print(p4)
  #print(p2)
  dev.off()
...
runcreateSignacObj2 <- function(dirIn,dirOut,prefix,genomeV){
  #library(Signac);library(EnsDb.Hsapiens.v86);library(EnsDb.Mmusculus.v79)
  # QC parameters
  #minPRK=3000  # minimum of fragments in peaks
  #maxPRK=20000 # max of fragments in peaks
  ...
  metadata <- read.csv(file = "per_barcode_metrics.csv", header = TRUE,  row.names = 1)
  ...
  setwd(dirOut)
  #pdf(paste0("01_1_plot_tss_",prefix,".pdf"))
  png(paste0("01_1_plot_tss_",prefix,".png"),height = 600, width = 600)
  print(p4)
  dev.off()
...
# maxPRK=10000 or sd3 # max of fragments in peaks
# PctRiP=15  # Fraction of fragments in peaks    
# blr=0.05 # Ratio reads in genomic blacklist regions
# nsr=4  # the ratio of mononucleosomal to nucleosome-free fragments per cell
# tssE=2 # Transcriptional start site (TSS) enrichment score
...
runQCSignacObj <- function(objAt,dirOut,prefix){
  ...
  titlecap<-paste0("Flitering: Pct_read_peak>15",",min_ncountPeak=",minPRK,",max_ncountPeak=",maxPRK,
                   ",TSSenrichment>2",",Blacklist ratio<0.05","nucleosome_signalRatio<2")
  qcCells<-dim(objAt)[2]
  p4<-VlnPlot(object = objAt,
              features = c('pct_reads_in_peaks', 'peak_region_fragments',
                           'TSS.enrichment', 'blacklist_ratio', 'nucleosome_signal'),
              pt.size = 0,  ncol = 5) + labs(subtitle=prefix, caption=titlecap)
  setwd(dirOut)
...
runCellProportion <- function(objectA,prefix,dirOut,cType){
  ...
  }else if(cType=="celltype"){
    metaTbl  <- data.frame(table(objectA$celltype))
  ...
  p<- ggplot(metaTblA, aes(x=cluster, y=Proportion, fill=cluster)) + ylab("Proportion (%)") +xlab("Cluster")+
  ...
  p1<- ggplot(metaTblA, aes(x=cluster, y=Proportion, fill=cluster)) + ylab("Proportion (%)") +xlab("Cluster")+
    geom_bar(stat="identity") + geom_text(aes(label=cells), vjust=-0.3, size=5) +
    scale_x_discrete(limits = rev(levels(metaTblA$cluster)))+ coord_flip()+
...
# p <- runCellProportion(obj.WTp14,"test",dirOut = dirOut,cType = "ID")
...
# rnaCellType="celltype" # celltype info column name in scRNA
# parameters: fragPath-frgment file path
runCoembedscRnaAtacmm10 <- function(scRNA,scAtac,dirOut,prefix,rnaCellType="celltype",fragPath){
  print(scRNA); print(scAtac)
  scRNA$seqType  <- "scRNA"
  scAtac$seqType <- "scATAC"
...
# rnaCellType="celltype" # celltype info column name in scRNA
# parameters: fragPath-frgment file path
runCoembedscRnaAtacHg38woFrag <- function(scRNA,scAtac,dirOut,prefix,rnaCellType="celltype"){
  print(scRNA); print(scAtac)
  scRNA$seqType  <- "scRNA"
  scAtac$seqType <- "scATAC"
...
run_dotplot4HAScelltypeMkers<- function(objectA,prefix){
  ...
  #chkMkList<- readRDS("/storage/chen/data_share_folder/22_10x_hs_AnteriorSegment_data/scAtacQC/data/obj_marker_HAS_list_8T.rds")
  chkMkList<- readRDS("/storage/chen/data_share_folder/22_10x_hs_AnteriorSegment_data/marker_genes_HAS/obj_marker_HAS_list_6T.rds")
  ...
  for (i in 1:length(tNames2))
  {
    ...
    p<-DotPlot(object = objectA,features = chkMkList2, dot.scale = 6, assay = "RNA",col.min = 0, scale.max = 80,
            cols = c("gray","magenta1")) + RotatedAxis() + theme(axis.text.x = element_text(angle = 45,hjust=1)) +
            labs(title=paste0("Markers for ",tType," - ",prefix))
    #    labs(caption=paste0("-Data: ",prefix,", markers for ",tType))
    #p2<-list(p2,p)
    p2[[i]] <- p
...
run_dotplot8HAScelltypeMkers<- function(objectA,prefix){
  # load HAS marker object
  #mkers    <- readRDS("/storage/chen/data_share_folder/22_10x_hs_AnteriorSegment_data/scAtacQC/data/obj_mkers_HAS_mkerSets.rds")
  chkMkList<- readRDS("/storage/chen/data_share_folder/22_10x_hs_AnteriorSegment_data/marker_genes_HAS/obj_marker_HAS_list_8T.rds")
...
runReclust_has <- function(objectA,prefix, dirOut,rsN,dn){
...
# parameters: objectA-seurat object, prefix, rsN:resolution, dirOut-output dir 
...
runDEGs <- function(objectA, prefix, dirOut){
  ...
  print(DotPlot(object = objectA, features = rev(x = unique(aTop5$gene)), dot.scale = 8, assay = "RNA",
                col.min = 0) + RotatedAxis())
  dev.off()
  ...
  print(DotPlot(object = objectA, features = rev(x = unique(aTop10$gene)), dot.scale = 8, assay = "RNA",
                col.min = 0) + RotatedAxis())
  dev.off()
...
# draw feature plot for RPE, fibroblast cells
# usage: checkRPEclusterHS(objA,prefix,dirOut)

checkRPEclusterHS <- function(objA,prefix,dirOut){
  # plot -markers
  p1 <- FeaturePlot(object = objA, features = c("RPE65","COL3A1"), pt.size=.1, min.cutoff = 0, ncol = 2)
  
  setwd(dirOut)
  #pdf(paste0("2.1_plot_umap_feature_markers_d20_rs_",rsN,"_",prefix,"_6.pdf"),width=7,height=12)
  png(paste0("03_1_plot_feature_markers_RPE_choroid_",prefix,".png"),width=700,height=300)
  print(p1)
  dev.off()
}

checkTSSfeatureHS <- function(objA,prefix,dirOut){
  # plot -markers
  p1 <- FeaturePlot(object = objA, features = c("pct_reads_in_peaks","peak_region_fragments","TSS.enrichment","TSS.percentile","nucleosome_signal","nucleosome_percentile"), pt.size=.1, min.cutoff = 0, ncol = 2)
  setwd(dirOut)
  #pdf(paste0("2.1_plot_umap_feature_markers_d20_rs_",rsN,"_",prefix,"_6.pdf"),width=7,height=12)
  png(paste0("03_0_plot_feature_TSSenrichment_",prefix,".png"),width=700,height=900)
  print(p1)
  dev.off()
}


# function : get predicted celltype list per cluster
# parameter: outCross table
# usage: ct <- get_predictedCelltypeByscPred(outCross)
get_predictedCelltypeByscPred <- function(outCross){
  # remove unassined data
  outCross2 <- outCross[which(rownames(outCross) != "unassigned"), ]
  outCross3 <- t(outCross2)
  # get celltype by max
  rmaxNames <- rownames(outCross2)[apply(outCross2,2,which.max)]
  
  ct=NULL
  for (i in 1:dim(outCross3)[1]){
    celltype <- colnames(outCross3)[which.max(outCross3[i,])]
    print(celltype)
    outCross3 <- outCross3[,which(colnames(outCross3) != celltype)]
    ct <-c(ct,celltype)
    i= i-1
  }
  print(ct)
