...
# creat outdir
# process
dirOut <- "out_check_knownMarkers_comb"
dirOut <- setDir(dirIn, dirOut)

# read rds
setwd(dirIn)
schard::h5ad2seurat(finrds)
#sceasy::convertFormat(finrds, from="anndata", to="seurat",outFile='tm_comb.rds')
obj_data <- readRDS("tm_comb.rds")
...
# dotplot
p2<- DotPlot(object = obj_data,features = mkers, dot.scale = xl, assay = "RNA",col.min = 0, 
